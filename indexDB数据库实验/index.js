...
function showAllData(){
	...
	getConnection(function(db){
        ...
        arrs.onsuccess = function(event){
        	var leght = 0;
        	var array = event.target.result;console.log(array);
	...
	getConnection(function(db){
        ...
        arrs.onsuccess = function(event){
        	...
        	for(var i in array){
        		...
        		if(reg.test(array[i].name)||reg.test(array[i].sex)||reg.test(array[i].tel)||reg.test(array[i].age)){
        			createRow();
	        		var tr = tbody.getElementsByTagName('tr');
...
operaBtn[1].onclick = function(){
	...
	for(var i = 0; i < tr.length; i++){
		...
		if(check.checked){
			...
			tbody.removeChild(check.parentNode.parentNode);
		}
	}
}
//按条件查询数据库中学生
